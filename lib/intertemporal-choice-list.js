const maxRowNumber = 14;
var amountBreak = 5;
var maxRowAmount = maxRowNumber*amountBreak;

const dateColor = 'color:#11439e'

var amountList,clickedOptionType,clickedAmount,allChoicesChecked;
var currentRow = 0;

function updateCurrentRow(amountList){
    currentRow = amountList.indexOf(clickedAmount)
...
function optionAmounts(start){
    var amountList = [];
    for (var x = 0; x <= maxRowNumber; x++) {
            amountList.push(start+amountBreak*x);
    }
...
function certaintyOptions(){
    ...
    for (var x = 0; x <= amountList.length-1; x++) {
            optionAList.push(`get <span style=${dateColor}>£${amountList[x]}</span> with certainty`);
    }
    return(optionAList);   
}
...
function generatePriceList(optionAContent,optionBContent) {
    ...
    for (var x = 0; x <= amountList.length-1; x++) {
        ...
        // optionA.id = 'constantOption';
        // if (optionA) {
        //     optionA.innerHTML = optionAContent; 
        // }
        // document.getElementById("content-container").appendChild(optionA)
    }
